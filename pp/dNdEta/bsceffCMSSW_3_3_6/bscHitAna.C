...
void compareBsc(TTree* BscTree, TTree* BscTree2,const char *cut,const char *title, char *projectTitle)
{
  ...
  c2->Print(Form("cnhit_%s.eps",title));
  ...
  c3->Print(Form("cnzp_%s.eps",title));
  ...
  cpm->Print(Form("cnpm_%s.eps",title));

  int tsize=16;
  TPaletteAxis *palette = new TPaletteAxis(tsize*1.02,0,tsize*1.1,tsize,hpm);
  ...
  c95pm->Print(Form("c95npm_%s.eps",title));
...
void bscHitAna(char *infile="output/treeBscEff100.root",char* infile2="output/treeBscEff95.root", char *projectTitle = "pythis900GeV",string source="mc")
{
   ...
   string trig="(nzp>=1 && nzm>=1)";
   evtType.push_back("inelastic"); evtTypeCut.push_back("1==1");
   evtType.push_back("inelastic_bit40"); evtTypeCut.push_back(trig);
   if (source=="mc") {
     evtType.push_back("SD"); evtTypeCut.push_back("(evtType==92||evtType==93)");
     evtType.push_back("NSD"); evtTypeCut.push_back("(evtType!=92 && evtType!=93)");
     evtType.push_back("SD_bit40"); evtTypeCut.push_back(trig+"&& (evtType==92||evtType==93)");
     evtType.push_back("NSD_bit40"); evtTypeCut.push_back(trig+"&& (evtType!=92 && evtType!=93)");
   }
