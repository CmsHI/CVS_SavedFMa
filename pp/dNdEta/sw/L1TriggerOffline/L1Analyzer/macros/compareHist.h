...
class compareHist
{
   public:
       ...
       compareHist(TTree *myTree1,TTree *myTree2,const char *var,const char *cut1,const char *cut2,double low=0,double high=0,int nbin=100,int normType=2);
       ...
       void Draw(TString opt) { h1->Draw(opt); h2->Draw(opt+"same"); if(leg1) leg1->Draw();}
       void Draw2(TString opt) { h2->Draw(opt); h1->Draw(opt+"same"); if(leg1) leg1->Draw();}
...
compareHist::compareHist(TTree *myTree1,TTree *myTree2,const char *var,const char *cut1,const char *cut2,double low,double high,int nbin,int normType)
{
   if (low==high) {
       low = myTree1->GetMinimum(var);
       if (low>myTree2->GetMinimum(var)) low = myTree2->GetMinimum(var);
   ...
   h2 = new TH1F(Form("h2%s",var),"",nbin,low,high);
   myTree1->Draw(Form("%s>>h%s",var,var),cut1);
   myTree2->Draw(Form("%s>>h2%s",var,var),cut2);
