void count(const char * inf0name = "jet_ana_v3_2_et_order_2.8TeV.root",
  Double_t NEt = 100.,
  Double_t AEt = 90.,
  Double_t DPhi = 2.2,
  Double_t EtaCut = 2.
  )
{
  ...
  TString djCut(Form("(njet>%.1f && ajet>%.1f && dphi>%.2f && abs(njeta)<%.1f && abs(ajeta)<%.1f)",NEt,AEt,DPhi,EtaCut,EtaCut));
