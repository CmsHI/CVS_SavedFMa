...
if isData:
   ...
   ivars.inputFiles = "file:/mnt/hadoop/cms/store/user/yetkin/MC_Production/Pythia50_HydjetDrum_mix01/RECO/set1_random10000_HydjetDrum_1.root"
...
process.load('Saved.QM12Ana.ExtraTrackReco_cff')
...
##################### Track Analyzers
process.load("Saved.QM12Ana.TrkAnalyzers_cff")
...
process.reco_extra        = cms.Path( process.siPixelRecHits * process.siStripMatchedRecHits *
                                      ...
                                      process.PFTowers *
                                      process.hiextraTrackReco * process.hiSelectedTrackHighPurity
                                      )
...
if (isData):
   ...
   process.ana_step.replace(process.anaTrack,process.trackeff_seq*process.anaTrack)
   process.ana_step+=(process.genpana + process.HiGenParticleAna)
#    process.anaTrackGeneral = process.anaTrack.clone(trackSrc = "hiMergedTracksGeneral")
#    process.ana_step+=process.anaTrackGeneral
#    process.anaTrackSelected = process.anaTrack.clone(trackSrc = "hiMergedTracksSelcted")
#    process.ana_step+=process.anaTrackSelected
