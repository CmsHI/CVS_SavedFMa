...
void drawInclJetResultSys_Overlay_hlt80NoCaloSkim(int fragMode= 2, // 1=trkpt, 2=ff
...
{ 
  ...
  int doCompare=2; // 1=qm12, 2=paper data, 3=Feb04

  TString tag = Form("trkPtProj_binMode2_hlt80NoCaloSkim_hi_rewt%d_pp_sm%drewt%d_bc%d_mc%d",jtrewthi,ppsm,jtrewtpp,biascorr,doMC);
  ...
  if (doCompare==2) {
    cmpFile_hi=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Mar19job0_data_hlt80NoCaloSkim_hi_corrjbias%d_v3.root",biascorr);
    cmpFile_pp=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Mar19job0_data_mc80to170_hi_pp_corrjbias%d_v3.root",biascorr);
  } else if (doCompare==3) {
  ...
  if (doCompare==1) {
    ...
    for ( int iaj=1;iaj<=4;iaj++) {
      ffhicmp[ijet][iaj]  = (TH1D*)load(cmpFile_hi.Data(),Form("hpt_%s_sigTrk_hidata_icent%d_irj999_fragMode%d_closure100_rewt0_ppsm0_wtmode0_pt1to300",jname[0].Data(),iaj,fragMode));
  ...
  if (!doMC) {
    ...
    else if (doCompare==2) l2[ijet]->AddEntry(ffratiocmp[ijet][1],"PbPb (No Calo90Skim)","p");
    ...
    else if (doCompare==2) l2[ijet]->AddEntry(ffppcmp[ijet][1],"pp (No Calo90Skim)","l");
  ...
  for ( int iaj=1 ; iaj<=4 ; iaj++) {
    ...
    } else if ( binMode == 2 ) drawText(Form("%.0f%% - %.0f%%", float(centBinAna[iaj-1]*2.5), float(centBinAna[iaj]*2.5)), 2,0.03,kBlack,25,false);
