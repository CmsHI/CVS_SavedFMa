...
void drawInclJetResultSys_Overlay_vs_Atlas(int fragMode= 2, // 1=trkpt, 2=ff
...
{ 
  ...
  TString tag = Form("trkPtProj_binMode2_hi_rewt%d_pp_sm%drewt%d_bc%d_vs_atlas",jtrewthi,ppsm,jtrewtpp,biascorr);
  // tag+="_eta1.2";
  ...
  // std::string Inputpp_=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Mar04v3job0_data_ppfitrewt_corrjbias%d_v3.root",biascorr);
  ...
  // for ( int iaj=1;iaj<=4;iaj++) {
  //   if (cmpStyle==1) ffratiocmp[ijet][iaj]->Divide(ffhicmp[ijet][4]);
  //   else if (cmpStyle==2) ffratiocmp[ijet][iaj]->Add(ffhicmp[ijet][4],-1);
  // }
  ...
  makeMultiPanelCanvasNew(c,1,2,0.0,0.0,0.15,0.22,0.01,1.0,0.95);
  int iaj=1;
  ...
  if (doCompare) ffhicmp[ijet][4]->Draw("hist same");
  ffhi[ijet][4]->Draw(   "hist same");
  ...
  if (cmpStyle==1) ffratio[ijet][iaj]->Divide(ffhi[ijet][4]);
  else if (cmpStyle==2) ffratio[ijet][iaj]->Add(ffhi[ijet][4],-1);

  if(ijet == 1)hPadR->Draw();
  sysff.Combine(ffhi[ijet][iaj],sysff.vErrorRat[ijet][iajSys],sysff.NErrorRatio);
  sysff.ApplyOnRel(ffhi[ijet][iaj],ffhi[ijet][4],cmpStyle);
