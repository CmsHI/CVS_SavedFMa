...
void sysJetAlgo() {
  TH1::SetDefaultSumw2();
//   std::string Input_="dijetFF_output_histograms_trkPtProjectOnJetAxis_trackPtCut1_pfJet.root";
//   std::string InputSys_="dijetFF_output_histograms_trkPtProjectOnJetAxis_trackPtCut1_calo.root";
  std::string Input_="dijetFF_output_histograms_trkPtProjectOnJetAxis_trackPtCut1_FinaletaCut2.00_pfJet.root";
  std::string InputSys_="dijetFF_output_histograms_trkPtProjectOnJetAxis_trackPtCut1_FinaletaCut2.00_calo.root";

  TH1D* ff[3][5]; // (leading, subleading),  (aj bin)
  TH1D* ffsys[3][5];
  
  TFile * outf = new TFile("systematics/jetalgo_sys.root","recreate");
  ...
  for (int binMode=2; binMode<=2; ++binMode) {  // 1 : aj, 2 : cent
    ...
    ff[ijet][iaj]  = (TH1D*)load(Input_.data(),Form("hpt_%s_sigTrk_hidata_icent%d_irj999_fragMode2_closure100",jetName.Data(),iaj));
    ffsys[ijet][iaj]  = (TH1D*)load(InputSys_.data(),Form("hpt_%s_sigTrk_hidata_icent%d_irj999_fragMode2_closure100",jetName.Data(),iaj));
        } 
