...
//         ffpp[ijet][iaj]  = (TH1D*)load(Input_.data(),Form("hpt_%s_sigTrk_ppdata_icent%d_irj999_fragMode2_closure%d",jname[ijet].Data(),iaj,refMode));
      	...
      	else ffhi[ijet][iaj]  = (TH1D*)load(Input_.data(),Form("hpt_jet_sigTrk_hidata_icent%d_irj999_fragMode2_closure100_jtrewt1_wtmode0_pt1to300",iaj));
        ffpp[ijet][iaj]  = (TH1D*)load(Input_.data(),Form("hpt_jet_sigTrk_ppdata_icent%d_irj999_fragMode2_closure100_jtrewt1_wtmode0_pt1to300",iaj));
      } 
...
//   TString cmpFile = "inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_Jan17data_hi_pprewt.root";
//   for ( int ijet=1; ijet<=1; ijet++){ 
//     for ( int iaj=1;iaj<=4;iaj++) {
...
//       drawSysErr(ffratiocmp[ijet][iaj],vError[ijet][iajSys],Nerror,ijet-1,1,2,1,0,0,0);
  ...
  drawText("CMS Preliminary",0.2,0.88,1,25); 
	drawText("L_{Int} = 150 #mub^{-1}",0.6,0.88,1,25);
