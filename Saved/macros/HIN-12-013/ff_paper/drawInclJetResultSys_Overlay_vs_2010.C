...
void drawInclJetResultSys_Overlay_vs_2010(int fragMode= 2, // 1=trkpt, 2=ff
...
{ 
  ...
  int doCompare=0; // 1=qm12, 2=paper data, 3=2010 data
  bool drawQM12 = true;

  TString tag = Form("trkPtProj_binMode2_0to4_hi_rewt%d_pp_sm%drewt%d_bc%d",jtrewthi,ppsm,jtrewtpp,biascorr);
  // tag+="_eta1.2";
  ...
  // Specify Inputs
  //////////////////////////////////////////////////////////////////////
  // std::string Input_="inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_Feb14v2data_mc80to170_hi_pp.root";
  // std::string Inputpp_="inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_Feb14v2data_mc80to170_hi_pp.root";
  // std::string Input_=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Feb14v2_data_mc80to170_hi_pp_corrjbias%d_v2.root",biascorr);
  // std::string Inputpp_=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Feb14v2_data_mc80to170_hi_pp_corrjbias%d_v2.root",biascorr);
  std::string Input_=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Mar04v3job0_data_mc80to170reco_hi_pp_corrjbias%d_v3.root",biascorr);
  std::string Inputpp_=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Mar04v3job0_data_mc80to170reco_hi_pp_corrjbias%d_v3.root",biascorr);
  // track pt rebin
  // std::string Input_=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Mar04v3job0_data_mc80to170reco_hi_pp_corrjbias%d_v3_trkptrebin.root",biascorr);
  // std::string Inputpp_=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Mar04v3job0_data_mc80to170reco_hi_pp_corrjbias%d_v3_trkptrebin.root",biascorr);
  // mc
  if (drawQM12) {
    Input_="dijetFF_output_histograms_trkPtProjectOnJetAxis_trackPtCut1_FinaletaCut2.00_pas.root";    
    Inputpp_="dijetFF_output_histograms_trkPtProjectOnJetAxis_trackPtCut1_FinaletaCut2.00_pas.root";    
  }
  if (doMC) {
    // Input_="inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_Feb14v2data_mc80to170_hi_pp.root";
    // Inputpp_="inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_Feb14v2data_mc80to170_hi_pp.root";
    // Input_=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Feb14v2_data_mc80to170_hi_pp_corrjbias%d_v2.root",biascorr);
    // Inputpp_=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Feb14v2_data_mc80to170_hi_pp_corrjbias%d_v2.root",biascorr);
  ...
  TString cmpFile_hi = Input_QM.data(), cmpFile_pp = Input_QM.data();
  if (doCompare==2) {
    // cmpFile_hi = "inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_Feb14v2data_mc80to170_hi_pp.root"; // latest data
    // cmpFile_pp = "inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_Feb14v2data_mc80to170_hi_pp.root"; // latest data
    // cmpFile_hi=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Feb14v2_data_mc80to170_hi_pp_corrjbias%d_v2.root",biascorr);
    // cmpFile_pp=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Feb14v2_data_mc80to170_hi_pp_corrjbias%d_v2.root",biascorr);
    cmpFile_hi=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Mar04v3job0_data_mc80to170_hi_pp_corrjbias%d_v3.root",biascorr);
    cmpFile_pp=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Mar04v3job0_data_mc80to170_hi_pp_corrjbias%d_v3.root",biascorr);
  }
  ...
  for ( int iaj=1;iaj<=4;iaj++) {
    if (!drawQM12) {
      ...
      if (fragMode==2) {
        ffhi[ijet][iaj]  = (TH1D*)load(cmpFile_hi.Data(),Form("hpt_%s_sigTrk_hidata_icent%d_irj999_fragMode2_closure100_wtmode0_pt1to500",jname[ijet].Data(),iaj));
        ffpp[ijet][iaj]  = (TH1D*)load(cmpFile_pp.Data(),Form("hpt_%s_sigTrk_ppdata_icent%d_irj999_fragMode2_closure100_wtmode0_pt1to500",jname[ijet].Data(),iaj));
      } else if (fragMode==1) {
        GetQM12TrkPt(ffpp,ffhi);
      }    
    }    
  ...
  if (doCompare==1) {
    ...
    if (fragMode==2) {
      for ( int iaj=1;iaj<=4;iaj++) {
        ffhicmp[ijet][iaj]  = (TH1D*)load(cmpFile_hi.Data(),Form("hpt_%s_sigTrk_hidata_icent%d_irj999_fragMode2_closure100_wtmode0_pt1to500",jname[ijet].Data(),iaj));
        ffppcmp[ijet][iaj]  = (TH1D*)load(cmpFile_pp.Data(),Form("hpt_%s_sigTrk_ppdata_icent%d_irj999_fragMode2_closure100_wtmode0_pt1to500",jname[ijet].Data(),iaj));
    ...
    for ( int iaj=1;iaj<=4;iaj++) {
      ffhicmp[ijet][iaj]  = (TH1D*)load(cmpFile_hi.Data(),Form("hpt_%s_sigTrk_hidata_icent%d_irj999_fragMode%d_closure100_rewt1_ppsm0_wtmode0_pt1to300",jname[0].Data(),iaj,fragMode));
      ffppcmp[ijet][iaj]  = (TH1D*)load(cmpFile_pp.Data(),Form("hpt_%s_sigTrk_ppdata_icent%d_irj999_fragMode%d_closure100_rewt1_ppsm%d_wtmode0_pt1to300",jname[0].Data(),iaj,fragMode,ppsm));
      ffratiocmp[ijet][iaj]  = (TH1D*)ffhicmp[ijet][iaj]->Clone(Form("%s_ratcmp",ffhicmp[ijet][iaj]->GetName()));
      if (cmpStyle==1) ffratiocmp[ijet][iaj]->Divide(ffppcmp[ijet][iaj]);
      else if (cmpStyle==2) ffratiocmp[ijet][iaj]->Add(ffppcmp[ijet][iaj],-1);
      handsomeTH1(ffppcmp[ijet][iaj],kRed,0,0);
  ...
  // Draw Result
  /////////////////////////////////////////////////
  // TCanvas *c = new TCanvas("c","",500,860);
  // makeMultiPanelCanvasNew(c,1,2,0.0,0.0,0.22,0.22,0.01,1.0,0.95);
  ...
  if (!drawQM12) {
    ...
    drawQM12TrackPtRatioSysBand_0to4();
  }
  ...
  TLegend* l2[3];
  TLegend* lsys;
  if (fragMode==2) l2[ijet]  = new TLegend(0.163,0.725,0.90,0.894,NULL,"brNDC");
  ...
  if (!doMC) {
    ...
    // l2[ijet]->AddEntry(ffhi[ijet][1],"PbPb/pp Inclusive Jet","");
    if (!drawQM12) l2[ijet]->AddEntry(ffratio[ijet][1],"2011, 0-10%, Inclusive jet","p");
    else l2[ijet]->AddEntry(ffratio[ijet][1],"2011, 0-10%, Inclusive jet","");
    if (doCompare==3) {
      // l2[ijet]->AddEntry(xsin4_1_0_R,"PbPb/pp Leading Jet","");
      l2[ijet]->AddEntry(xsin4_1_0_R,"2010, 0-30%, Leading jet","p");
    }
    // l2[ijet]->AddEntry(ffpp[ijet][1],"pp reference","l");
    // if (doCompare==1) l2[ijet]->AddEntry(ffppcmp[ijet][1],"pp reference QM12","l");
    // else if (doCompare==3) l2[ijet]->AddEntry(xsin4_1_0_R,"pp Leading Jet (2010)","l");
  } else {
  ...
  l2[ijet]->Draw();
  if (fragMode==2) drawText("Jet p_{T}  > 100 GeV/c",0.31,0.64,kBlack,25);
  else if (fragMode==1) {
    drawText("Jet p_{T}  > 100 GeV/c",0.41,0.65,kBlack,25);
    drawText("Track p_{T}  > 1 GeV/c",0.41,0.59,kBlack,25);
  }
  if (!doMC) {
    drawText("L_{Int} = 150 #mub^{-1}",0.64,0.87,1,22);
    if (drawQM12) drawText("CMS Preliminary",0.56,0.22,1,25); 
  }
  // for ( int iaj=1 ; iaj<=4 ; iaj++) {
  //   c->cd(5-iaj);
