...
void drawInclJetResultSys_Overlay_RatioClosure(int fragMode= 2, // 1=trkpt, 2=ff
...
{ 
  ...
  // Specify Inputs
  //////////////////////////////////////////////////////////////////////
  // std::string   Input_=Form("plotsFinalFF/FFana2_trkPtProj_binMode2_gluon_hi_rewt0_pp_sm2rewt0_bc11_mc0_rat_EtaRef.root");
  // std::string   Inputpp_=Form("plotsFinalFF/FFana2_trkPtProj_binMode2_gluon_hi_rewt0_pp_sm2rewt0_bc0_mc1_rat_EtaRef.root");
  std::string   Input_=Form("plotsFinalFF/FFana2_trkPtProj_binMode2_pyquenwide_hi_rewt0_pp_sm2rewt0_bc11_mc0_rat_EtaRef.root");
  std::string   Inputpp_=Form("plotsFinalFF/FFana2_trkPtProj_binMode2_pyquenwide_hi_rewt0_pp_sm2rewt0_bc0_mc1_rat_EtaRef.root");
  TString tag = TString(Input_.c_str()).ReplaceAll(".root","_RatioClosure");
  ...
  TString cmpFile_hi=Input_.c_str();
  TString cmpFile_pp=Inputpp_.c_str();
  ...
  for ( int iaj=1;iaj<=4;iaj++) {
    ffhi[ijet][iaj]  = (TH1D*)load(Input_.data(),Form("ffRattio_ijet1_iaj%d",iaj));
    ffpp[ijet][iaj]  = (TH1D*)load(Inputpp_.data(),Form("ffRattio_ijet1_iaj%d",iaj));
  ...
  if (doCompare) {
    for ( int iaj=1;iaj<=4;iaj++) {
      ffhicmp[ijet][iaj]  = (TH1D*)load(cmpFile_hi.Data(),Form("hpt_jet_sigTrk_himc_icent%d_irj999_fragMode2_closure100_rewt0_ppsm0_wtmode0_pt1to300_ratcmp",iaj));
      ffppcmp[ijet][iaj]  = (TH1D*)load(cmpFile_pp.Data(),Form("hpt_jet_sigTrk_himc_icent%d_irj999_fragMode2_closure101_rewt0_ppsm0_wtmode0_pt1to300_ratcmp",iaj));
  ...
  //////////////////////////////////////////////////////////////////////
  float xmin=-0.15, xmax=5.5, rmin=0., rmax=2;
  ...
  hPadFF = new TH2F("hPadFF",";#xi = ln(1/z);PbPb/pp;",100,xmin,xmax,100,0.,2.58);
  hPadR = new TH2F("hPadR",";#xi = ln(1/z);Reco/Truth",100,xmin,xmax,20000,rmin,rmax);
  ...
  for ( int iaj=1 ; iaj<=4 ; iaj++) {
    ...
    // gPad->SetLogy();
    sysff.Combine(ffhi[ijet][iaj],sysff.vErrorHi[ijet][iajSys],sysff.NErrorRatio);
  ...
  TLegend* l2[3];
  TLegend* lsys;
  l2[ijet]  = new TLegend(-0.04,0.71,0.7,1.02,NULL,"brNDC");
  easyLeg(l2[ijet],"",23);
  if (!doMC) {
    l2[ijet]->AddEntry(ffhi[ijet][1],"Reco PYQUEN^{Wide}+HYDJET","p");
    // l2[ijet]->AddEntry(ffhi[ijet][1],"Reco PYTHIA^{gluon}+HYDJET","p");
    if (doCompare==1) l2[ijet]->AddEntry(ffratiocmp[ijet][1],"Reco PYTHIA+HYDJET","p");
    // else if (doCompare==2) l2[ijet]->AddEntry(ffratiocmp[ijet][1],"PYTHIA+HYDJET Truth","p");
    l2[ijet]->AddEntry(ffpp[ijet][1],"Truth PYQUEN^{Wide}+HYDJET","l");
    // l2[ijet]->AddEntry(ffpp[ijet][1],"Truth PYTHIA^{gluon}+HYDJET","l");
    // if (doCompare==1) l2[ijet]->AddEntry(ffppcmp[ijet][1],"PYTHIA","l");
    if (doCompare==1) l2[ijet]->AddEntry(ffppcmp[ijet][1],"Truth PYTHIA+HYDJET","l");
  } else {
  ...
  for ( int iaj=1 ; iaj<=4 ; iaj++) {
    ...
    } else if ( binMode == 2 ) drawText(Form("%.0f%% - %.0f%%", float(centBin1[iaj-1]*2.5), float(centBin1[iaj]*2.5)), 2,0.18,kBlack,25,false);
  ...
  c->SaveAs(Form("%s.gif",tag.Data()));
  c->SaveAs(Form("%s.pdf",tag.Data()));
  // if (saveOutput==1) {
