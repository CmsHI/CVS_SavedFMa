...
void drawClosuresSmear() {
  ...
  // Plot4x4::title1="PYTHIA";
  // Plot4x4::title2="PYTHIA Unsmeared";
  // Inputs
  // // mc
  // smear vs unsmeared pp
  // infname  = "inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_Jan17mc80and120_pp.root";
  ...
  for ( int icent=1; icent<=4 ; icent++) {
    h[ijet][icent]    = (TH1D*)load(infname, Form("hjetPt_hi%s_icent%d_irj999_fragMode2_closure102_jtrewt0_",dataset.Data(),icent));
    href[ijet][icent] = (TH1D*)load(reffname,Form("hjetPt_pp%s_icent%d_irj999_fragMode2_closure102_jtrewt0_",dataset.Data(),icent));
  }
  ...
  for (int ia=0; ia<2; ++ia) {
    for (int isub=2; isub<3; ++isub) {
      ...
      for ( int icent=1; icent<=4 ; icent++) {
        h[ijet][icent]    = (TH1D*)load(infname,  Form("hpt_jet_%sTrk_hi%s_icent%d_irj999_fragMode%d_closure102_jtrewt0_wtmode0_pt1to300",sub[isub].Data(),dataset.Data(),icent,ana[ia]));
        href[ijet][icent] = (TH1D*)load(reffname, Form("hpt_jet_%sTrk_pp%s_icent%d_irj999_fragMode%d_closure102_jtrewt0_wtmode0_pt1to300",sub[isub].Data(),dataset.Data(),icent,ana[ia]));
      }
