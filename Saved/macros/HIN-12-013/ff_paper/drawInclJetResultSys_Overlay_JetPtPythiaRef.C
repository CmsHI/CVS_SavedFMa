...
void drawInclJetResultSys_Overlay_JetPtPythiaRef(int fragMode= 2, // 1=trkpt, 2=ff
...
{ 
  ...
  int doCompare=3; // 1=qm12, 2=paper data, 3=ppdata ref

  // TString tag = Form("trkPtProj_binMode2_JetPt100to120_hi_rewt%d_pp_sm%drewt%d_bc%d",jtrewthi,ppsm,jtrewtpp,biascorr);
  TString tag = Form("trkPtProj_binMode2_JetPt100to120_hi_rewt%d_ppmc_sm%drewt%d_bc%d",jtrewthi,ppsm,jtrewtpp,biascorr);
  // tag+="_eta1.2";
  ...
  // Specify Inputs
  //////////////////////////////////////////////////////////////////////
  std::string Input_=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to120eta2.00_jdr0.3_Mar19job0_data_mc80to170_hi_pp_corrjbias%d_v3.root",biascorr);
  std::string Inputpp_=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to120eta2.00_jdr0.3_Mar19job0_data_mc80to170_hi_pp_corrjbias%d_v3.root",biascorr);
  
  // mc
  if (doMC) {
    Input_=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to120eta2.00_jdr0.3_Mar19job0_data_mc80to170_hi_pp_corrjbias%d_v3.root",biascorr);
    Inputpp_=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to120eta2.00_jdr0.3_Mar19job0_data_mc80to170_hi_pp_corrjbias%d_v3.root",biascorr);
  ...
  if (doCompare==2) {
    cmpFile_hi=Form("inclJetFF_output_trackPtCut1_FinalJetPt100to300eta2.00_jdr0.3_Mar19job0_data_mc80to170_hi_pp_corrjbias%d_v3.root",biascorr);
    ...
    cmpFile_hi=Input_.data();
    cmpFile_pp=Inputpp_.data();
  }
  ...
  for ( int iaj=1;iaj<=4;iaj++) {
    if ( binMode == 2 ) {
      ...
      ffpp[ijet][iaj]  = (TH1D*)load(Inputpp_.data(),Form("hpt_%s_sigTrk_ppmc_icent%d_irj999_fragMode%d_closure100_rewt%d_ppsm%d_wtmode0_pt1to300",jname[0].Data(),iaj,fragMode,jtrewtpp,ppsm));
  ...
  if (!doMC) {
    ...
    l2[ijet]->AddEntry(ffpp[ijet][1],"PYTHIA reference","l");
    ...
    else if (doCompare==3) l2[ijet]->AddEntry(ffppcmp[ijet][1],"pp (data)","l");
  } else {
  ...
  for ( int iaj=1 ; iaj<=4 ; iaj++) {
    ...
    if (iaj==1) {
      ...
      drawText(Form("Jet 100 < p_{T}  < 120 GeV/c"),ptx,pty1,kBlack,25);
