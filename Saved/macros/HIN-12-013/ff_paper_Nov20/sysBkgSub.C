...
void sysBkgSub() {
  ...
  std::string InputSys_="dijetFF_output_histograms_trkPtProjectOnJetAxis_trackPtCut1.root";

  TH1D* ff[3][5]; // (leading, subleading),  (aj bin)
  TH1D* ffsys[3][5];
  
  TFile * outf = new TFile("systematics/bkgsub_sys.root","recreate");
  ...
  for (int binMode=2; binMode<=2; ++binMode) {  // 1 : aj, 2 : cent
    ...
    ffsys[ijet][iaj]  = (TH1D*)load(InputSys_.data(),Form("hpt_%s_sigTrk_hidata_icent%d_irj999_fragMode2_closure0",jetName.Data(),iaj));
        } 
