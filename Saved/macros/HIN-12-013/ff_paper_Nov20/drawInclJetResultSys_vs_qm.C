...
void drawInclJetResultSys_vs_qm(int binMode =2, // 1 : aj, 2 : cent
  bool doEtaRef = true ) 
{  
  TH1::SetDefaultSumw2();
  std::string Input_="inclJetFF_output_trackPtCut1_FinaletaCut2.00_Nov20.root";
  std::string Inputpp_="inclJetFF_output_trackPtCut1_FinaletaCut2.00_Nov20pp.root";
//   std::string Inputpp_="inclJetFF_output_trackPtCut1_FinaletaCut2.00_Nov20ppnosmear.root";
  ...
  for ( int ijet=1; ijet<=1; ijet++){ 
    for ( int iaj=1;iaj<=4;iaj++) {
      if ( binMode == 2 ) {
      	if (!doEtaRef) ffhi[ijet][iaj]  = (TH1D*)load(Input_.data(),Form("hpt_%s_sigTrk_hidata_icent%d_irj999_fragMode2_closure0_wtmode0_pt1to300",jname[0].Data(),iaj));
      	else ffhi[ijet][iaj]  = (TH1D*)load(Input_.data(),Form("hpt_%s_sigTrk_hidata_icent%d_irj999_fragMode2_closure100_wtmode0_pt1to300",jname[0].Data(),iaj));
        ffpp[ijet][iaj]  = (TH1D*)load(Inputpp_.data(),Form("hpt_%s_sigTrk_ppdata_icent%d_irj999_fragMode2_closure100_jtrewt1_wtmode0_pt1to300",jname[0].Data(),iaj));
      } 
...
//     l2[ijet]->AddEntry(ffratiocmp[ijet][1],"Jet |#eta|<0.8","p");
    l2[ijet]->AddEntry(ffratiocmp[ijet][1],"QM12","p");
...
// 	drawText("L_{Int} = 129 #mub^{-1}",0.27,0.80,1,25);
	drawText("L_{Int} = 150 #mub^{-1}",0.27,0.80,1,25);
      }
      float legdx = 0.2;
