...
void sysJetRes() {
  ...
  std::string InputSys_="dijetFF_output_histograms_trkPtProjectOnJetAxis_trackPtCut1_additionPbPbSmear.root";

  TH1D* ff[3][5]; // (leading, subleading),  (aj bin)
  TH1D* ffsys[3][5];
  
  TFile * outf = new TFile("systematics/jetres_sys.root","recreate");
  ...
  for (int binMode=2; binMode<=2; ++binMode) {  // 1 : aj, 2 : cent
    ...
    for ( int ijet=1; ijet<=1; ijet++){ 
      for ( int iaj=1;iaj<=4;iaj++) {
        ...
        if      ( binMode == 1 ) {
    ff[ijet][iaj]  = (TH1D*)load(Input_.data(),Form("hpt_%s_sigTrk_hidata_icent999_iaj%d_fragMode2_closure0",jetName.Data(),iaj));
