...
//   std::string InputSys_="dijetFF_output_histograms_trkPtProjectOnJetAxis_trackPtCut1_FinaletaCut2.00_pfJet.root";
  ...
  TFile * outf = new TFile("systematics/closure_sys.root","update");
  ...
  for (int binMode=2; binMode<=2; ++binMode) {  // 1 : aj, 2 : cent
    ...
    ff[ijet][iaj]  = (TH1D*)load(Input_.data(),Form("hpt_%s_sigTrk_himc_icent%d_irj999_fragMode2_closure102",jetName.Data(),iaj));
    ffsys[ijet][iaj]  = (TH1D*)load(InputSys_.data(),Form("hpt_%s_sigTrk_himc_icent%d_irj999_fragMode2_closure100",jetName.Data(),iaj));
        } 
