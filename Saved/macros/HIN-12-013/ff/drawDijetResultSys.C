...
{  
  ...
  for ( int ijet=1; ijet<=1; ijet++){
    for ( int iaj=1;iaj<=4;iaj++) {
      if (binMode==1) {
        ...
        jesError[ijet][iaj] =  loadError(JESfile,Form("hpt_%s_sigTrk_hidata_icent%d_irj999_fragMode2_closure100_sys7",jname[ijet].Data(),iaj));
      ...
      for(int i = 1; i <= constError[ijet][iaj]->GetNbinsX(); ++i){
        constError[ijet][iaj]->SetBinContent(i,1.04);
  ...
  for ( int ijet=1; ijet>=1 ; ijet--) {
    for ( int iaj=1 ; iaj<=4 ; iaj++) {
      ...
      gPad->SetLogy();      
      drawSysErr(ffhi[ijet][iaj],vError[ijet][iajSys],Nerror,ijet-1,1,2,1,ijet == 1,0,0);
      ...
      drawSysErr(ffratio[ijet][iaj],vError[ijet][iajSys],Nerror,ijet-1,1,2,1,ijet == 1,0,0,(TPad*)c->GetPad(9-iaj),(TPad*)cerr->GetPad(5-iaj));
