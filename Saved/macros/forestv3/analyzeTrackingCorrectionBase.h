...
class TrkCorrHisAna
{
  public:
    ...
    void FillRecHistograms(const EvtSel & evt, const RecTrack_t & r);
    void FillSimHistograms(const EvtSel & evt, const SimTrack_t & s);
};
...
void TrkCorrHisAna::FillRecHistograms(const EvtSel & evt, const RecTrack_t & r)
{
   float zvar = r.jet;
   if (trkPhiMode_) zvar = r.phir;

   // monitor
   vhTrkJetPtDr[0]->Fill(deltaR(r.etar,r.phir,eta1,phi1),r.ptr);
   vhTrkJetPtDr[1]->Fill(deltaR(r.etar,r.phir,eta2,phi2),r.ptr);
...
void TrkCorrHisAna::FillSimHistograms(const EvtSel & evt, const SimTrack_t & s) {
   float zvar = s.jet;
   if (trkPhiMode_) zvar = s.phis;

   if(s.status>0) {
      // monitor
      vhSimJetPtDr[0]->Fill(deltaR(s.etas,s.phis,eta1,phi1),s.pts);
      vhSimJetPtDr[1]->Fill(deltaR(s.etas,s.phis,eta2,phi2),s.pts);
