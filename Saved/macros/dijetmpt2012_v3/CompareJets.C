...
//    TString infrefname = "../ntout/output-hy18dj80_forest2_v0_xsec_icPu5.root";
   ...
   for (int i=0; i<6; ++i) {
      ...
      if (i==0) {
         ...
         cmp.Init(40,-2,2);
         cmp.Project("eta1>>");
         cmp.Draw(";Leading Jet #eta;u.n.",0,0.08);
      } else if (i==5) {
         cmp.vsel.push_back(mycut&&sigSel);
         cmp.vsel.push_back(mycut&&sigSel);
         c2->SetLogy(0);
         cmp.Init(40,-3.14159,3.14159);
         cmp.Project("phi1>>");
         cmp.Draw(";Leading Jet #phi;u.n.",0,0.06);
      }
...
//       if (i!=0) cmp.leg->AddEntry(cmp.vh[0],Form("%.0f - %.0f \%",cMin*2.5,cMax*2.5),"");
      ...
      if (i!=1) cmp.leg->AddEntry(cmp.vh[0],"#Delta#phi_{1,2} > #frac{7}{8}#pi","");
