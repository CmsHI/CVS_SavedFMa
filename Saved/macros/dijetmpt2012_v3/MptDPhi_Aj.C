                    ...
                    )
{
   ...
   for (int a=0; a<nAjBin; ++a) {
      ...
      for (int i=0;i<nptrange+1;i++) {
         pe[i]=new TH1D(Form("p%d_a%d",i,a),"",ndphibin,dphibins);
      }
   }
   // Get Values
   for (int a=0; a<nAjBin; ++a) {
      if (a!=nAjBin-2) continue;
      for (int idr=0; idr<ndphibin; ++idr) {
         float sum=0, sumerr=0;
         for (int i=0;i<nptrange;i++) {
            TString hname = insrc+Form("_pt%d_dphi%d",i,idr);
...
//             cout << hname << " " << hMptAj << endl;
   ...
   pall->SetXTitle("#Delta#phi(j1,trk)");
...
//    drawText("(d)",0.04,0.95);
   ...
   c1->SaveAs(Form("%s/missingPtParallel-TrkHP-CorrRes%d-data-mcRec-DPhi.pdf",outdir.Data(),doResCorr));
   c1->SaveAs(Form("%s/missingPtParallel-TrkHP-CorrRes%d-data-mcRec-DPhi.gif",outdir.Data(),doResCorr));
}
