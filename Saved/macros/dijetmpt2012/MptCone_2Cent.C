                    ...
                    )
{
   ...
   for (int i=0;i<nBin+1;i++)
   {
      ...
      for (int a=0; a<nBinAj; ++a) {
         TString hname = Form("%s%s%.0f_merge%d_3SigAll",insrc.Data(),metType.Data(),(a+1)*4.,i);
         if (a==3) hname = Form("%sAllAcc_merge%d_3SigAll",insrc.Data(),i);
...
//   TString inputFile_Py="fig/04.18_pythia_reco_gen/HisOutput_DiJetv7_v4_icPu5_Mpt_120_50_2618_Norm0_c0_40.root";
   ...
   c1->cd(1);
   balanceMetVsAj(inputFile_Py,"hypho_mptxgenpSig","InCone",false,false);
//   balanceMetVsAj(inputFile_Py,"hypho","InCone",false,false);
...
//   drawText("(b)",0.04,0.91);
   ...
   c1->cd(3);
   TCut phiSysCut;
   balanceMetVsAj(inputFile_peripheral,"hi_mptxtrkCorr","InCone",false);
   ...
   c1->cd(4);
   balanceMetVsAj(inputFile_central,"hi_mptxtrkCorr","InCone",false);
   drawText("0-30%",0.7,0.93);
   drawText("(d)",0.04,0.95);
   gPad->RedrawAxis();
   
   c1->SaveAs(Form("missingPtParallel-Corrected-data-mcgen-ConeSize.pdf"));
   c1->SaveAs(Form("missingPtParallel-Corrected-data-mcgen-ConeSize.gif"));
}
