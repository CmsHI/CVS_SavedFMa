...
public:
   ...
   void Init(int nbins, float * bins) {
      for (int i=0; i<vt.size(); ++i) {
         ...
         vh.push_back(new TH1D(Form("%s_%d",name.Data(),i),"",nbins,bins));
         vnorm.push_back(1.);
      }
   }
   ...
   void Project(TString var) {
      ...
      for (int i=0; i<vt.size(); ++i) {
         vt[i]->Draw(Form("%s%s",var.Data(),vh[i]->GetName()),sel&&vsel[i],"goff",100000);
...
//       int vstyl[10] = {kFullCircle,kOpenCircle,kOpenCircle,kOpenCircle,kOpenCircle};
   ...
   TH1D * Ratio(TString ytitle="ratio",int inum=0, int iden=1) {
      if (vh.size()<2) {
         cout << "not enough histograms to divide" << endl;
         return 0;
      }
      TH1D * hr = (TH1D*)vh[inum]->Clone(Form("%s_div_%s",vh[inum]->GetName(),vh[iden]->GetName()));
      hr->Divide(vh[iden]);
      hr->SetYTitle(ytitle);
      //hr->Fit("pol1","");
      return hr;
