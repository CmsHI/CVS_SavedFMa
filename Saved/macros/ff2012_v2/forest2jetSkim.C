...
void forest2jetSkim(TString inputFile_="mc/photon50_25k.root", std::string outname = "barrelPhoton50_25k.root", bool needReweight=true, int maxEvents=-1, TString MinbiasFname="", bool isMC=false, TString jetAlgo="akPu3PF", 
  ...
  if  (MinbiasFname != "") {
    ...
    tdj->Branch("indiJet",&dj.pthat,"pthat/F:jetPt:jetEta:jetPhi:jetPtGM:jetEtaGM:jetPhiGM");
  }
  ...
  for (Long64_t jentry=startEntry ; jentry<nentries;jentry++) {
    ...
    // jet energy smearing is done 

    /* remove sub leading jet finder 
       for (int ij=0; ij< nJets ; ij++) {
       float theJtpt ;
       float theJteta;
       float theJtphi;
    ...
    if ( dj.lJetPt < InclusiveJetPtCut ) 
    continue;
	...
	nTrk++;
      ...
      /* commented out particle flow part 
