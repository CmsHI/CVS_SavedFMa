    ...
    Int_t normType=0)
{
  ...
  c1->cd(2);
  drawTrkEnergy("plot/drawn_jfh"+anaV+"_Hydjet_"+module+"_Cent0to30_Aj0to24_"+BckSub+title+".root",false,false,logScale,normType);
  ...
  c1->cd(4);
  drawTrkEnergy("plot/drawn_jfh"+anaV+"_PyquenUQ80_"+module+"_Cent0to100_Aj24to100_"+BckSub+title+".root",false,true,logScale,normType);
  drawText("PYTHIA",leftSpace+0.43,0.51);
  drawText("A_{J}>0.24",leftSpace+0.43,0.43);

  c1->cd(5);
  drawTrkEnergy("plot/drawn_jfh"+anaV+"_Hydjet_"+module+"_Cent0to30_Aj24to100_"+BckSub+title+".root",true,false,logScale,normType);
  drawText("Hydjet",0.43,downSpace+0.51);
  drawText("+PYTHIA",0.43,downSpace+0.43);
  drawText("0-30%",0.43,downSpace+0.35);
  drawText("A_{J}>0.24",0.43,downSpace+0.27);
  ...
  c1->Print(anaV+"_"+BckSub+title+"TrackEnergyPtRBkgSubAjAll32_"+drawV+Form("_norm%d.gif",normType));
  c1->Print(anaV+"_"+BckSub+title+"TrackEnergyPtRBkgSubAjAll32_"+drawV+Form("_norm%d.eps",normType));
  c1->Print(anaV+"_"+BckSub+title+"TrackEnergyPtRBkgSubAjAll32_"+drawV+Form("_norm%d.C",normType));
}
