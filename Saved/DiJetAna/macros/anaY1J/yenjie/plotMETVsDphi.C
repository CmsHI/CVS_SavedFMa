...
void plotMETVsDphi()
{
   ...
   tGen2->SetName("gen2");
   TFile *inf4 = new TFile("nt_dj_data100_cor_05.root");
   TTree *tRecoData = (TTree*) inf4->FindObjectAny("ntjt");
...
//   double BinsMC[nBin+1] = {0+delta,1+delta,2+delta,2.2+delta,2.4+delta,2.6+delta,2.8+delta,3+delta,PI+delta};
   ...
   double BinsMC[nBin+1] = {2+delta,2.2+delta,2.4+delta,2.6+delta,2.8+delta,3+delta,PI+delta};
   ...
   TCut evtCut = "nljet>120&&abs(nljetacorr)<1&&aljet>50&&abs(aljetacorr)<1&&!maskEvt&&cent<30&&Aj>0.0";

   tGen->Draw("-gen.metx:jdphi+0.02>>p","weight"*evtCut);
   tGen->Draw("-gen2.metx:jdphi+0.02>>p2","weight"*evtCut);
   tGen->Draw("-reco.metx:jdphi+0.02>>p3","weight"*evtCut);
   tRecoData->Draw("-metx:jdphi>>p4","1"*evtCut);
   p->SetMarkerStyle(24);
