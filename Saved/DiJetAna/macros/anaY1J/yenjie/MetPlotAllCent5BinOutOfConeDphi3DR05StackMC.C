...
void MetPlotAllCent5BinOutOfConeDphi3DR05StackMC(char *inputFile="data.root")
{
   TCanvas *c1 = new TCanvas("c1","",1000,1000);
   Float_t leftMargin=0.28,bottomMargin=0.18;
   makeMultiPanelCanvas(c1,2,2,0.0,0.0,leftMargin,bottomMargin,0.02);
   c1->cd(1);
   balanceMetVsAj("nt_dj_HyUQ80v4_djcalo_genp_100_50_05.root","cent>=30","",false,false);

   drawText("PYTHIA+HYDJET",0.33,0.82);
   drawText("All Genparticles",0.33,0.74);
   drawText("Out of Cone |#Delta R|>0.5",0.33,0.66);

   drawText("30-100%",0.85,0.9);
   float ptx(0.32),pty(0.25);
   drawText("p_{T,1}  > 120GeV/c",ptx,pty);
   drawText("p_{T,2}  > 50GeV/c",ptx,pty-0.08);
   drawText("#Delta#phi_{12}>  3.0",ptx,pty-0.16);
   
   c1->cd(2);
   balanceMetVsAj("nt_dj_HyUQ80v4_djcalo_genp_100_50_05.root","cent<30","",true,false);

   drawText("0-30%",0.8,0.9);

   c1->cd(3);
   balanceMetVsAj("nt_dj_mix100_05.root","cent>=30","",false);
   drawText("PYTHIA+HYDJET reconstructed",0.33,0.90);
   drawText("30-100%",0.85,0.93);
   float pty1(0.4);
   drawText("p_{T,1}  > 120GeV/c",ptx,pty1);
   drawText("p_{T,2}  > 50GeV/c",ptx,pty1-0.07);
   drawText("#Delta#phi_{1,2}>  3.0",ptx,pty1-0.14);


   c1->cd(4);
   balanceMetVsAj("nt_dj_mix100_05.root","cent<30","",false);
//   balanceMetVsAj("nt_dj_HyUQ80v4_djcalo_genp_100_50.root","cent<30","",false);
   drawText("0-30%",0.8,0.93);
   
   c1->SaveAs("missingPtParallel-OutOfCone05-dphi3-Corrected-MCAllGenParticle-allCent.eps");
   c1->SaveAs("missingPtParallel-OutOfCone05-dphi3-Corrected-MCAllGenParticle-allCent.gif");
