		    ...
		    )
{
  ...
  TH1D * hMean = new TH1D("hMean",";<#slash{p}_{T}^{#parallel}>;sub experiment",30,-10,10);
  for (Int_t i=0; i<t.size(); ++i) {
    cout << "Sel evt: " << t[i]->GetEntries(evtCut&&myCut) << endl;
    t[i]->Project(vhMetx[i]->GetName(),"-metx",(evtCut&&myCut)*"weight");
    t[i]->Project(vhJPhi[i]->GetName(),"nljphi",(evtCut&&myCut)*"weight");
    //vhMetx[i]->Fit("gaus");
    Float_t mean = vhMetx[i]->GetMean();
    cout << "metx mean: " << mean << endl;
    hMean->Fill(mean);
    vhMetx[i]->Scale(1./vhMetx[i]->GetEntries());
    vhJPhi[i]->Scale(1./vhJPhi[i]->GetEntries());
  }

  TCanvas * c2 = new TCanvas("c2","c2",600,600);
  vhMetx[0]->SetAxisRange(0,0.1,"Y");
  vhMetx[0]->Draw("");
  for (Int_t i=1; i<t.size(); ++i) {
    vhMetx[i]->Draw("same hist");
  }

  TCanvas * c3 = new TCanvas("c3","c3",600,600);
  vhJPhi[0]->Draw("");
  vhJPhi[0]->SetAxisRange(0,0.1,"Y");
  for (Int_t i=1; i<t.size(); ++i) {
    vhJPhi[i]->Draw("same");
  }

  TCanvas * c13 = new TCanvas("c13","c13",600,600);
  hMean->Scale(1./hMean->GetEntries());
  hMean->SetAxisRange(0,0.4,"Y");
  hMean->Fit("gaus");
  hMean->Draw();
}
