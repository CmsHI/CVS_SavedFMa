    ...
    TString header="Hydjet+UQDiJet80to120")
{
  ...
  cpXiNr.AddHist1D(hgCm.H("XiNr"),"reco - j1","E",kBlack,kFullCircle);
  cpXiNr.AddHist1D(hgCm.H("Cm1XiNr"),"gen - j1","E",kRed,kOpenStar);
  cpXiNr.SetLegend(0.194,0.7,0.52,0.94);
  cXiNr->cd(1); cpXiNr.Draw((TPad*)cXiNr->GetPad(1),false);
  CPlot cpXiNrRat("XiNrRat","XiNrRat","#xi=ln(E_{T}^{Jet}/p_{T}^{trk})","reco/gen");
  ...
  cpPPt.AddHist1D(hgCm.H("PPtNr"),"reco j1","E",kGreen+2,kOpenTriangleUp);
  cpPPt.AddHist1D(hgCm.H("Cm1PPtNr"),"gen j1","E",kRed+2,kOpenDiamond);
  cpPPt.AddHist1D(hgCm.H("PPtAw"),"reco j2","E",kBlack,kFullCircle);
  cpPPt.AddHist1D(hgCm.H("Cm1PPtAw"),"gen j2","E",kRed,kOpenStar);
  cPPt->cd(1); cpPPt.Draw((TPad*)cPPt->GetPad(1),false);
  CPlot cpPPtRat("PPtRat","PPtRat","p_{T}^{trk}","reco/gen");
  cpPPtRat.AddHist1D(hgCm.R("PPtNrDivCm1PPtNr"),"j1",kGreen+2,kOpenSquare);
