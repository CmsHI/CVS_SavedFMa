		    ...
		    )
{
   ...
   // Selection cut
   TCut evtCut = "nljet>120&&abs(nljetacorr)<1.6&&aljet>50&&abs(aljetacorr)<1.6&&abs(jdphi)>5./6*TMath::Pi()&&!maskEvt";
...
void MetPlotInConeOutConeDPhiCut_RandomCone(char *inputFile="data.root")
{
   TCanvas *c1 = new TCanvas("c1","",1000,1000);
   Float_t leftMargin=0.28,bottomMargin=0.18;
   makeMultiPanelCanvas(c1,2,2,0.0,0.0,leftMargin,bottomMargin,0.02);
   c1->cd(1);
   balanceMetVsAj("../ntv6_datambv3mix1_djcalo_c0to30.root","cent<30","metConex",false,true);
   drawText("CMS 0-30%",0.33,0.82);
   drawText("In-Cone",0.85,0.9);
   drawText("#DeltaR<0.8",0.85,0.9-0.06);
   float ptx(0.32),pty(0.25);
   drawText("Random Cone (MB Sample)",ptx,pty);
   drawText("(a)",0.31,0.91);
   
   c1->cd(2);
   balanceMetVsAj("../ntv6_datambv3mix1_djcalo_c0to30.root","cent<30","metOutOfConex",true,true);
   ...
   c1->SaveAs("missingPtParallel-Corrected-data-InConeOutConeDPhiCut_RandomCone.eps");
   c1->SaveAs("missingPtParallel-Corrected-data-InConeOutConeDPhiCut_RandomCone.gif");
}
