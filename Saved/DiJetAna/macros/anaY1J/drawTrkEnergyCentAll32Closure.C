...
void drawTrkEnergyCentAll32Closure(
    ...
    Int_t normType=0)
{
  ...
  c1->cd(3);
  drawTrkEnergy("plot/drawn_jfh"+anaV+"_Hydjet_"+module+"_Cent0to10_Aj0to100_"+BckSub+title+".root",false,true,logScale,normType);
  drawText("0-10%",0.43,0.35);

  c1->cd(4);
  drawTrkEnergy("plot/drawn_jfh"+anaV+"_Hydjet_"+moduleGen+"_Cent30to100_Aj0to100_"+BckSub+title+".root",false,false,logScale,normType);
  drawText("30-100%",leftSpace+0.43,downSpace+0.51);
  //drawText("CMS",0.56,downSpace+0.80);
  drawText("Pythia+Hydjet Genp",0.56,downSpace+0.80);

  c1->cd(5);
  drawTrkEnergy("plot/drawn_jfh"+anaV+"_Hydjet_"+moduleGen+"_Cent10to30_Aj0to100_"+BckSub+title+".root",false,true,logScale,normType);
  drawText("10-30%",0.43,downSpace+0.51);

  c1->cd(6);
  drawTrkEnergy("plot/drawn_jfh"+anaV+"_Hydjet_"+moduleGen+"_Cent0to10_Aj0to100_"+BckSub+title+".root",false,false,logScale,normType);
