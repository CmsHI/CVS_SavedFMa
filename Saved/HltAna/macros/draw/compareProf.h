...
{
public:
   Compare(TString name, TTree * t1, TTree * t2, TCut s1, TCut s2, TString c="", int norm=0) :
   name1(name+"1"),name2(name+"2"),tag(name),
   sel1(s1), sel2(s2), cut(c),
   nSelEvt1(0), nSelEvt2(0), normMode(norm) {
      tr1 = t1;
      tr2 = t2;
   }
   ...
   void Init(int nbins, float xmin, float xmax) {
      h1 = new TProfile(name1,"",nbins,xmin,xmax);
      h2 = new TProfile(name2,"",nbins,xmin,xmax);      
   }
   void Init(int nbins, float * bins) {
      h1 = new TProfile(name1,"",nbins,bins);
      h2 = new TProfile(name2,"",nbins,bins);
   }
   
   void Project(TString var,TString opt="") {
      cout << var+name1 << " " << TString(sel1&&cut) << endl;
      cout << var+name2 << " " << TString(sel2&&cut) << endl;
      tr1->Draw(var+name1,sel1&&cut,opt+"goff");
      tr2->Draw(var+name2,sel2&&cut,opt+"goff");
   }
   ...
   void Draw(TString title, float ymin=0, float ymax=0, TString lname1="data", TString lname2="ref", bool drawLeg = true, float lx1=0.4, float ly1=0.18, float lx2=0.87, float ly2=0.28) {
      ...
      leg->AddEntry(h2,lname2,"p");
      if (drawLeg) leg->Draw();
   }
   
   TProfile * Ratio(TString ytitle="ratio") {
      TProfile * hr = (TProfile*)h1->Clone(name1+"_div_"+name2);
      hr->Divide(h2);
      hr->SetYTitle(ytitle);
      return hr;
   }
