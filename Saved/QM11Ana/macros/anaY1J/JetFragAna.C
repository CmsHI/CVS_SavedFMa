...
Int_t JetFragAna::GetEntry(Long64_t entry)
{
  ...
  // Got Entry
    ...
    p_.clear();
    if (!doJetOnly_) {
      for (Int_t i=0; i<anap_.np; ++i) {
	p_.push_back(math::PtEtaPhiMLorentzVector(anap_.ppt[i],anap_.peta[i],anap_.pphi[i],0.13957));
      }
    }
  } // finished with entry
  return result;
...
Int_t JetFragAna::GetJetEntry(TChain * t, AnaJet & jet, Long64_t entry)
{
  ...
  // Got Entry
    ...
    for (Int_t j=0;j<2;++j) anaJets_[j].SetCoordinates(jet.jtpt[j],jet.jteta[j],jet.jtphi[j],0);
    if (doEtaCorr_) {
      for (Int_t i=0;i<2;++i) {
...
void JetFragAna::SetJetTree(Int_t mode, TString tfile, TString tname)
{
  ...
  jetTree_[mode]->SetBranchAddress("jtpt",&vj_[mode].jtpt);
  jetTree_[mode]->SetBranchAddress("jteta",&vj_[mode].jteta);
  jetTree_[mode]->SetBranchAddress("jtphi",&vj_[mode].jtphi);
}
