    ...
    )
{
  ...
  // eff
  TH1D * hCorrPtRefJet100 = (TH1D*) trkCorrRef.InspectCorr(0,isample,centBeg,centEnd,ptBeg,ptEnd,3,7-etaPM,7+etaPM);
  handsomeTH1(hCorrPtRefJet100);
  hCorrPtRefJet100->SetAxisRange(40,499.9,"X");
  ...
  TH1D * hCorrPtJet40 = (TH1D*) trkCorr.InspectCorr(0,isample,centBeg,centEnd,ptBeg,ptEnd,3,7-etaPM,7+etaPM);
  hCorrPtJet40->Draw("same E");

  // fake
  TH1D * hCorr1PtRefJet100 = (TH1D*) trkCorrRef.InspectCorr(1,isample,centBeg,centEnd,ptBeg,ptEnd,3,7-etaPM,7+etaPM);
  hCorr1PtRefJet100->SetMarkerStyle(kOpenCircle);
  hCorr1PtRefJet100->Draw("sameE");
  TH1D * hCorr1PtJet40 = (TH1D*) trkCorr.InspectCorr(1,isample,centBeg,centEnd,ptBeg,ptEnd,3,7-etaPM,7+etaPM);
  hCorr1PtJet40->Draw("sameE");

  TLegend *leg = new TLegend(0.3,0.74,0.60,0.93);
  leg->SetFillStyle(0);
  leg->SetBorderSize(0);
  leg->SetTextSize(0.035);
  leg->AddEntry(hCorrPtRefJet100,"0-30%","");
  leg->AddEntry(hCorrPtRefJet100,"|#eta^{Trk}|<1, p_{T}^{Trk}>4","");
  leg->AddEntry(hCorrPtRefJet100,"Leading Jet","p");
  leg->AddEntry(hCorrPtJet40,"Subleading Jet","p");
  leg->Draw();

  cEff->Print("TrkCorr_vs_JetPt_Central.gif");
  cEff->Print("TrkCorr_vs_JetPt_Central.pdf");
}
