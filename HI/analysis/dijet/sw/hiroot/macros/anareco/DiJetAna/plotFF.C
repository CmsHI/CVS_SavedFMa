...
//   drawTree(trTruePythia, "padphi>>haPdPhiPythia","",drdbFF,"haPdPhiPythia","draw Pythia: dphi to leading partons",100,-0.5*PI,1.5*PI,false,kRed,7,3);
   ...
   int log=0;
   // -near-
   drawTree(trJetPythia, "log(1/zn)>>hXiNearJetPythia",pyt.GetCut().GetNJetPartlCut().Data(),drsgFFE,"hXiNearJetPythia",";(near) #xi=ln(E_{t}^{Jet}/E_{t}^{Particle});#frac{1}{N_{jet}} #frac{dN}{d#xi}",NXIBIN,0,XIMAX,log,kBlack,1,3,1,8,nJetPythiaNorm,XIYMAX);
   ...
   // -away-
   drawTree(trJetPythia, "log(1/za)>>hXiAwayJetPythia",pyt.GetCut().GetAJetPartlCut().Data(),drsgFFE,"hXiAwayJetPythia",";(away) #xi=ln(E_{t}^{Jet}/E_{t}^{Particle});#frac{1}{N_{jet}} #frac{dN}{d#xi}",NXIBIN,0,XIMAX,log,kBlack,1,3,1,8,nJetPythiaNorm,XIYMAX);
   ...
   //---FF ratio2---
   // -parton-
   drawDivHist("hXiNearPyquen","hXiNearPythia",drsgFF,"hNearFFRatio",";#xi;#frac{1}{N_{jet,pyquen}}#frac{dN}{d#xi_{pyquen}} / #frac{1}{N_{jet,pythia}}#frac{dN}{d#xi_{pythia}}",NXIBIN,0,XIMAX,0,kRed-2,1,3,0,0,3.);
   ...
   // -Jet-
   drawDivHist("hXiNearJetPyquen","hXiNearJetPythia",drdbFFE,"hNearJetFFRatio",";#xi;dN/d#xi_{pyquen} / dN/d#xi_{pythia}",NXIBIN,0,XIMAX,0,kBlue,1,3,1,8);
