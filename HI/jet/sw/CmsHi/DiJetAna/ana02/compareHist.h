...
class compareHist
{
   public:
       ...
       compareHist(TTree *myTree1,TTree *myTree2,const char *var,const char *name,TCut cut1,TCut cut2,double low=0,double high=0,int nbin=100)
       {
	 compareHist(myTree1,myTree2,var,name,TString(cut1).Data(),TString(cut2).Data(),low,high,nbin);
       }
